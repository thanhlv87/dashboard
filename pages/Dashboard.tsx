...
const UrgentTaskRow = ({ title, tag, date, status }: any) => (
  <>
    {/* Desktop view */}
    <div className="hidden md:flex items-center justify-between py-4 border-b border-border-color last:border-0 hover:bg-surface-light/30 px-2 rounded transition-colors">
    ...
    {/* Mobile view */}
    <div className="md:hidden flex flex-col gap-2 p-3 border-b border-border-color last:border-0 hover:bg-surface-light/30 rounded transition-colors">
      <div className="flex items-start justify-between gap-2">
        <span className="text-sm font-medium text-white flex-1">{title}</span>
        <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-surface-light text-white shrink-0`}>
          {tag}
        </span>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-xs text-[#fa5538] font-medium">{date}</span>
        <button className="text-xs font-bold text-white border border-border-color bg-transparent px-3 py-1 rounded-md hover:bg-surface-light transition-all">
          {status}
        </button>
      </div>
    </div>
...
const ScheduleItem = ({ day, date, title, time, onClick }: any) => (
  <div
    onClick={onClick}
    className="flex items-center gap-2 md:gap-3 p-2 hover:bg-surface-light/30 rounded-lg transition-colors cursor-pointer"
  >
    <div className="flex flex-col items-center justify-center bg-surface-light p-1.5 md:p-2 rounded-lg min-w-[44px] md:min-w-[50px] h-[48px] md:h-[54px]">
      <p className="text-white text-[10px] md:text-xs font-medium uppercase">{day}</p>
      <p className="text-white text-base md:text-lg font-bold leading-none">{date}</p>
    </div>
    <div className="flex flex-col justify-center flex-1 min-w-0">
      <p className="text-white font-medium text-xs md:text-sm line-clamp-1">{title}</p>
...
const StatCard = ({ icon, label, value, subValue, subColor }: any) => (
  <div className="flex flex-col gap-1 rounded-xl p-3 md:p-4 border border-border-color bg-surface">
    <div className="flex items-center gap-2 text-text-muted mb-1 md:mb-2">
      <span className="material-symbols-outlined text-sm md:text-base">{icon}</span>
      <p className="text-[10px] md:text-xs font-medium leading-normal">{label}</p>
    </div>
    <p className="text-white tracking-tight text-xl md:text-2xl font-bold leading-tight">{value}</p>
    <p className={`${subColor} text-[10px] md:text-xs font-medium leading-normal`}>{subValue}</p>
  </div>
);
...
const Dashboard = () => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <header className="sticky top-0 z-10 bg-background-dark/95 backdrop-blur-md border-b border-surface-light px-4 md:px-8 py-3 md:py-4 flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <h1 className="text-base md:text-xl font-bold text-white">Dashboard Tổng Hợp</h1>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <button className="size-8 md:size-10 rounded-lg bg-surface-light flex items-center justify-center text-white hover:bg-surface-light/80 transition-colors hidden md:flex">
            <span className="material-symbols-outlined text-[18px] md:text-[20px]">notifications</span>
          </button>
          <button className="size-8 md:size-10 rounded-lg bg-surface-light flex items-center justify-center text-white hover:bg-surface-light/80 transition-colors hidden md:flex">
            <span className="material-symbols-outlined text-[18px] md:text-[20px]">chat_bubble</span>
          </button>
          <div className="size-8 md:size-10 rounded-full bg-surface-light flex items-center justify-center text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col gap-4 rounded-xl border border-border-color p-6 bg-surface h-full">
              ...
              {tasksLoading ? (
                ...
                <div className="flex flex-col">
                  <div className="hidden md:flex items-center justify-between text-xs font-medium text-text-muted pb-3 border-b border-border-color">
          ...
          <div className="lg:col-span-1">
            <div className="flex flex-col gap-4 rounded-xl border-2 border-primary/80 p-5 bg-surface shadow-lg shadow-primary/5 h-full">
              <div className="flex bg-surface-light/30 p-0.5 md:p-1 rounded-lg">
                <button
                  onClick={() => setActiveTab('schedule')}
                  className={`flex-1 text-xs md:text-sm font-bold py-1.5 md:py-2 px-2 md:px-3 rounded-md transition-all ${activeTab === 'schedule' ? 'bg-surface-light text-white shadow-sm' : 'text-text-muted hover:text-white'}`}
                >
                  Lịch Giảng Sắp Tới
                </button>
                <button
                  onClick={() => setActiveTab('inventory')}
                  className={`flex-1 text-xs md:text-sm font-bold py-1.5 md:py-2 px-2 md:px-3 rounded-md transition-all ${activeTab === 'inventory' ? 'bg-surface-light text-white shadow-sm' : 'text-text-muted hover:text-white'}`}
                >
                  Tồn Kho
                </button>
              </div>
