                  ...
                  <tr key={product.id} className="border-b border-surface-light hover:bg-surface-light/30">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className={`h-2.5 w-2.5 rounded-full ${product.status === 'in-stock' ? 'bg-green-500' :
...
// ========================================
const RevenueView = () => {
  const { data: allRevenueRecords, loading } = useFirestore<RevenueRecord>('revenue', [orderBy('date', 'desc')]);
  const [selectedYear, setSelectedYear] = useState(new Date().getFullYear());

  // Get available years from revenue records
  const availableYears = Array.from(
    new Set(allRevenueRecords.map(r => r.date.toDate().getFullYear()))
  ...
  // Filter revenue records by selected year
  const revenueRecords = allRevenueRecords.filter(r => r.date.toDate().getFullYear() === selectedYear);
    ...
    <div className="flex-1 p-8 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <div className="flex flex-1 flex-col gap-2 rounded-xl bg-surface p-6 border border-border-color">
          <p className="text-base font-medium leading-normal text-white">Tổng Doanh thu {selectedYear}</p>
